...
test('historical release code runs in a separate job without repository token permissions', () => {
	...
	assert.match(build, /composer install --no-interaction --prefer-dist/);
	assert.match(build, /pnpm check/);
	assert.match(build, /pnpm check:generated/);
	assert.match(build, /composer run phpcs/);
	assert.match(build, /pnpm release:verify/);
	...
	assert.match(
		build,
		/name: ran-emailoctopus-recovery-\$\{\{ github\.run_id \}\}/
	);
	assert.match(build, /overwrite: true/);
	assert.doesNotMatch(build, /ran-emailoctopus-recovery-.*run_attempt/);
});
...
test('write-capable recovery publisher starts fresh and consumes only the rebuilt artifact', () => {
	...
	assert.doesNotMatch(publisher, /ran-emailoctopus-recovery-.*run_attempt/);
	assert.doesNotMatch(publisher, /actions\/checkout@/);
	assert.doesNotMatch(
		publisher,
		/pnpm install|composer install|scripts\/create-release-assets\.sh|pnpm release:verify/
	);
});
